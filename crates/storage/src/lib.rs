...
//! behind the `sqlite` feature, on by default). M1 adds [`libsql`] (#24, this
//! crate, behind the `libsql` feature) and Postgres (#25) the same way.
//!
//! Backends share a common encode/decode layer ([`codec`]) so the wire format
//! of every row is identical: UUIDv7 as 16-byte BLOBs, RFC 3339 timestamps,
//! integer-packed permission bitsets. Only the driver-specific error mapping
//! (uniqueness violations, FK restrictions) is implemented per-adapter.
//!
...
//! (#25) will respect this by routing only one backend through.
//!
