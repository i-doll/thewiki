//! Integration coverage for the SQLite audit-log repository.
...
use common::{fresh_storage, make_namespace, make_page, make_user};
use serde_json::json;
use thewiki_core::{AuditLogId, PageId, Revision, UserId};
...
async fn list_filters_by_actor_action_and_time_range() {
    ...
    let time_only = storage
        ...
        .await
        .expect("time-only filter");

    assert_eq!(time_only.items.len(), 1);
    assert_eq!(time_only.items[0].actor_username, "bob");
    assert_eq!(time_only.items[0].action, "page.update");
}
...
#[tokio::test]
async fn filters_normalise_fractional_timestamps_before_sql_comparison() {
    let storage = fresh_storage().await;
    ...
    let listed = storage
        ...
        .list(
            AuditLogFilter {
                since: Some(
                    OffsetDateTime::parse("2026-01-01T00:00:00.5Z", &Rfc3339)
                        .expect("parse fractional timestamp"),
                ),
                ..AuditLogFilter::default()
            },
            None,
    ...
    assert_eq!(listed.items.len(), 1);
    assert_eq!(listed.items[0].action, "page.half");
}
...
async fn insert_raw_audit_row(
    storage: &thewiki_storage::sqlite::SqliteStorage,
    actor_username: &str,
    action: &str,
    created_at: &str,
) {
    let id = AuditLogId::new().into_uuid();
    let actor_id = UserId::new().into_uuid();
    let target_id = PageId::new().into_uuid();
    sqlx::query(
    ...
    .bind(actor_id.as_bytes().as_slice())
    .bind(actor_username)
    .bind(action)
    .bind(target_id.as_bytes().as_slice())
    .bind(format!("Main/{action}"))
    .bind(created_at)
