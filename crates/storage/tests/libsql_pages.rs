//! Integration coverage for [`LibsqlPageRepository`].
//!
//! Mirrors `sqlite_pages.rs` — each test boots a fresh in-memory libsql
//! instance, seeds the supporting rows, and exercises one method or pair of
//! methods.
...
use common_libsql::{fresh_storage, make_namespace, make_page};
...
async fn list_in_namespace_pages_across_cursors() {
    ...
    for n in 0..5u8 {
        ...
        ids.push(p.id);
        // Stagger so each row has a distinct created_at — the cursor is
