//! Integration coverage for [`LibsqlUserRepository`].
...
async fn password_hash_is_stored_opaque() {
    ...
    // Pull the column directly — the repo trait keeps password material out
    // of the domain model on purpose.
    let conn = storage.connection();
    let id_bytes = *user.id.as_uuid().as_bytes();
    let mut rows = conn
        .query(
            "SELECT password_hash FROM users WHERE id = ?1",
            [libsql::Value::Blob(id_bytes.to_vec())],
        )
        .await
    ...
    let hash: Option<String> = row.get(0).expect("password_hash");
