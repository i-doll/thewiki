//! Integration coverage for [`SqlitePageRepository`].
//!
//! Each test boots a fresh in-memory database, seeds the supporting rows it
//! needs (a namespace, an author for revisions where relevant), then exercises
//! one method or pair of methods. The helper in `common::fresh_storage` does
//! the migrate dance once per test.
...
async fn update_missing_page_is_not_found() {
    ...
    let ns = make_namespace("main");
    storage.namespaces().create(&ns).await.expect("seed ns");
    let page = make_page(ns.id, "ghost"); // never inserted
...
async fn list_in_namespace_pages_across_cursors() {
    ...
    // Five pages; we'll page through two at a time.
    let mut ids = Vec::new();
    for n in 0..5u8 {
        let p = make_page(ns.id, &format!("page-{n}"));
        storage.pages().create(&p).await.expect("create");
        ids.push(p.id);
        // Tiny stagger to give each row a distinct created_at — the cursor is
        // `(created_at, id)` and equal timestamps would collapse into a single
        // group.
