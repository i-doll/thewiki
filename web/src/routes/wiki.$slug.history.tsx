...
/**
 * History view for a page. Renders the revisions newest-first using
 * `useInfiniteQuery` to walk the cursor returned by the API.
 *
 * Revert is gated on a placeholder "logged in" check (presence of a user-id in
 * localStorage). Until #11 lands the revert button is force-disabled via
 * {@link REVERT_ENABLED} — keeping the UI honest rather than firing requests
 * against a 404.
 */
...
/**
 * Flip to `true` once the revert endpoint (#11) ships. Until then we render
 * the button as disabled with a tooltip explaining why — cleaner than failing
 * requests every click.
 */
...
async function fetchRevisions(
	...
	const url = `/api/v1/pages/${encodeURIComponent(slug)}/revisions?${params.toString()}`;
	const res = await fetch(url);
	if (!res.ok) {
		// Best-effort body parsing so the UI can surface a server-supplied
...
async function postRevert(
	...
	const res = await fetch(`/api/v1/pages/${encodeURIComponent(slug)}/revert`, {
		method: "POST",
...
function isLoggedIn(): boolean {
	...
	try {
		...
		// localStorage may throw in private modes; treat as logged out.
		return false;
	}
}

/**
 * Format a UTC ISO timestamp as a coarse-grained relative string ("3 hours
 * ago"). Uses the native `Intl.RelativeTimeFormat` so we don't pay the
 * bundle cost of `date-fns` for a one-off.
 */
...
function HistoryComponent() {
	...
	const query = useInfiniteQuery({
		...
		queryFn: ({ pageParam }) => fetchRevisions(slug, pageParam),
		initialPageParam: undefined as string | undefined,
		getNextPageParam: (last) => last.next_cursor ?? undefined,
	});

	// Flatten the cursor-paginated pages into a single array for rendering.
	// `useMemo` keeps the reference stable so the list doesn't re-render the
	// whole DOM on unrelated parent updates.
	...
	const revertMutation = useMutation({
		mutationFn: (args: { revisionId: string; message: string | null }) =>
			postRevert(slug, args.revisionId, args.message),
		onSuccess: async () => {
			setRevertError(null);
			await queryClient.invalidateQueries({ queryKey: ["page", slug] });
			await queryClient.invalidateQueries({ queryKey: ["revisions", slug] });
			// `/wiki/$slug` is owned by #18 and may not exist as a typed route
			// in this branch yet. Use an untyped href so the build stays clean
			// regardless of merge order; the router resolves the path at
			// runtime once #18 lands.
			navigate({ href: `/wiki/${encodeURIComponent(slug)}` }).catch(() => {
				// Swallow navigation errors — falling back to the history page
				// after a successful revert is acceptable degradation.
			});
		},
		onError: (err: unknown) => {
	...
	const onRevert = (revision: RevisionView) => {
		...
		// `window.prompt` returns `null` when the user cancels — abort the
		// mutation entirely. Empty string means "no message" — let it through.
			...
			{revisions.length > 0 && (
				<ol className="flex flex-col gap-3">
					{revisions.map((rev, index) => {
						// `parent_id` is the diff partner for "diff with previous". For
						// the oldest revision in a history it's `null` — disable that
						// link rather than navigating to an invalid URL.
						const previousId = rev.parent_id;
						// "diff with current" pairs this revision against the head. The
						// head is whatever the first item is in the (newest-first) list
						// — caveat: the head may not yet be loaded if the user is deep
						// into pagination. We fall back to the first item we *have*.
